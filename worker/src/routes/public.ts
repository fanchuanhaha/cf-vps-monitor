...
function readRecoverySecretKey(body: Record<string, unknown>): string {
  const value = body.recovery_key ?? body.service_role_key;
...
publicRoutes.post('/admin/recovery', async (c) => {
  ...
  if (userCount === 1) {
    if (!serviceRoleKey || serviceRoleKey.length > MAX_ADMIN_RECOVERY_KEY_LENGTH) {
      return c.json({ error: 'Recovery key 无效' }, 400);
    }
    if (!await timingSafeEqualString(serviceRoleKey, c.env.JWT_SECRET?.trim() || '')) {
      return c.json({ error: 'Recovery key 无效' }, 403);
    }
  }
  ...
  runLoginBackground(
    ...
    db.insertAuditLog(
      ...
      username,
      'admin_recovery',
      userCount === 0 ? '首次创建管理员账号' : '通过 JWT Secret 重置管理员账号',
      'warning',
    ),
  );
