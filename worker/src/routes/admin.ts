...
async function buildHealthCheck(c: AdminContext, deep: boolean, cacheState: HealthCheckBody['cache']): Promise<{ body: HealthCheckBody; status: 200 | 503 }> {
  ...
  const databaseProbe = healthEvent('database_connection_probe', 'ok', 'D1 database configured', checkedAt);
  ...
  if (databaseProbe.status !== 'error') {
    components.database_role_probe = healthEvent('database_role_probe', 'disabled', 'Direct database role probe is not applicable in D1 mode', checkedAt);
    components.agent_token_hygiene_probe = await runAgentTokenHygieneProbe(database, checkedAt);
    components.schema_probe = healthEvent('schema_probe', 'disabled', 'Schema is handled by D1 migrations, not Worker runtime bootstrap', checkedAt);
  }
