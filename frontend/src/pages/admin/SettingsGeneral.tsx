...
const D1_PAID_STORAGE_BYTES = 50 * 1024 * 1024 * 1024;
...
export default function SettingsGeneral() {
  ...
  const derived = useMemo(() => {
    ...
    const d1PaidStorageBytes = capacity?.quota_reference?.database?.storage_bytes?.pro_project_reference ||
      D1_PAID_STORAGE_BYTES;
            ...
            <Box className="quota-estimate-panel quota-estimate-panel-embedded">
              <Flex align="start" justify="between" gap="3" wrap="wrap" mb="3">
                <Flex direction="column" gap="1" style={{ minWidth: 0, flex: '1 1 360px' }}>
                  ...
                  <Text size="1" color="gray" className="quota-reference-line">
                    历史存储按 D1 项目容量估算；Worker Free {formatInteger(derived.workerFreeDailyRequests)}/天，Paid {formatInteger(derived.workerPaidDailyRequests)}/天。
                  </Text>
                </Flex>
              ...
              <div className="quota-estimate-bar-grid">
                <QuotaBar
                  ...
                  caption={`D1 Free 存储参考 ${formatBytes(derived.freeStorageBytes)}，Paid 参考 ${formatBytes(derived.d1PaidStorageBytes)}`}
              ...
              <Text size="1" color="gray" style={{ display: 'block', marginTop: 8 }}>
                保存时会按允许范围校验并归一化；现在不再按数据库读写配额估算，主要关注 Worker 请求量和 D1 历史存储增长。
